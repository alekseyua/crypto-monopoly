...
export const GameBoard: React.FC<IGameBoard> =({ 
	...
	return (
		<div 
			ref={innerRef}
			className={cls['game-board']}
		>
			<div className={cls['game-board__info']}>
				{ActionCard  && ActionCard}
				<GameInfoBoardCorners 
					isGrayBlur={!!listSelectUserPreview.length}
				/>
			</div>
                    ...
                    return (
						<GameFieldNew
							...
							direction={
								...
								// 20 parcing
								// 30 police
								card.card_number >= 2 && card.card_number <= 10 ? 'bottom'
							...
							name={card.name}
							owner={card?.owner}
							onField={!!card.players.length && card.players.map( (p:IPlayer) =>  p.current_move)[0]}
							isGrayBlur={!!listSelectUserPreview.length && !listSelectUserPreview.includes(card.owner.player.id)}
							activeCardForSelect={!!listSelectUserPreview.length && listSelectUserPreview.includes(card.owner.player.id)}
						/>
                    );
                })
			}
