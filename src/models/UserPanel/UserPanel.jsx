...
const UserPanel = ({ userInfo, profile, isQG, user, handleNavigateTo, handleOpenDropDownMenu, isDropDownMenuOpen }) => {
  return (
    <div className={`${styles['header-menu-btn__user-panel-container']}`} >
      <div className={`${isQG ? styles.quickGame : styles['header-menu-btn__user-panel-container']} ${isDropDownMenuOpen && styles['open']}`} >
        {
          !!profile.id && userInfo?.map((info, i) => {
            // !!user?.username && userInfo?.map( (info, i) => {
            const { name, price, link, id } = info;
            return (
              <div
                key={i}
                onClick={() => isQG ? handleOpenDropDownMenu() : handleNavigateTo(link)}
                className={`${styles['header-menu-btn__container']} ${isQG && styles['containerButtonQG']}`}
                style={{
