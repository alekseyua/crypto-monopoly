...
import { ICard, IDataContainer, IDataQG, IInfoMassagePopup, IPlayer, ISpecialCard, IUserActions } from '../../../store/quick-game/quick-game.d';
...
export const FieldQGContainer: React.FC<IFildQG> = () => {
	...
	useEffect(() => {
		...
		const getAction = logFunc(function (player: IPlayer, data: IDataContainer): keyPreview {
			...
			// для экспресса 
			if (isExpressAirlineCruise) return {
		...
		const card = quickGame.cards.filter((el: ICard | ISpecialCard) => el.id === dataAction.cardId)[0];
